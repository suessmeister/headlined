...
export default function ArsenalPage() {
   ...
   const devWallet = new PublicKey('6PEe9kXaNXckNAQcgyKxGFjaoVxsSbwniv6xjFuLVrHq')
   ...
   async function buyGun(index: number) {
      ...
      const tx = new Transaction().add(paymentIx, createMintIx, mintIxInit, ataIx, mintToIx, ixMint)
                  ...
                  <div className="flex items-center justify-center min-h-screen">
                     <div className="bg-white p-6 rounded shadow-lg">
                        ...
                        <div className="mt-4 flex justify-end space-x-2">
                           ...
                           <button onClick={confirmPurchase} className="px-4 py-2 bg-green-500 text-white rounded">Confirm</button>
                        </div>
                     </div>
                  </div>
               </Dialog>
                        ...
                        {activeTab === 'shop' && (
                           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                              {sniperData.sniper.map((gun, index) => (
                                 <div key={index} className="transform transition-all duration-300 hover:scale-105" onClick={() => openModal(index)}>
