...
const City: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const zoomCheck = () =>
      setHideChars(Math.abs(window.devicePixelRatio - originalZoom) > 0.1);
    ...
    const handleMouseMove = (ev: MouseEvent) =>
      setZoomPos({ x: ev.clientX, y: ev.clientY });
    ...
    const drawWin = (bx: number, by: number, bw: number, bh: number) => {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          if (wx + wW > bx + bw - margin || wy + wH > by + bh - margin)
            continue;
