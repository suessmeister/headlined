...
import {
  getSnipeSweepersProgram,
  getSnipeSweepersProgramId,
} from "@project/anchor";
...
export function useSnipeSweepersProgram() {
  ...
  const programId = useMemo(
    () => getSnipeSweepersProgramId(cluster.network as Cluster),
    [cluster],
  );
  const program = useMemo(
    () => getSnipeSweepersProgram(provider, programId),
    [provider, programId],
  );
  ...
  const initialize = useMutation({
    ...
    mutationFn: (keypair: Keypair) =>
      program.methods
        .initialize()
        .accounts({ snipe_sweepers: keypair.publicKey })
        .signers([keypair])
        .rpc(),
    onSuccess: (signature) => {
...
export function useSnipeSweepersProgramAccount({
  account,
}: {
  account: PublicKey;
}) {
  ...
  const closeMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.close().accounts({ snipe_sweepers: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accounts.refetch();
    },
  ...
  const decrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.decrement().accounts({ snipe_sweepers: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
  ...
  const incrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.increment().accounts({ snipe_sweepers: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
  ...
  const setMutation = useMutation({
    ...
    mutationFn: (value: number) =>
      program.methods.set(value).accounts({ snipe_sweepers: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
