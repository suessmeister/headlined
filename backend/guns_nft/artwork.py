...
def get_fonts():
    ...
    try:
        title_font = ImageFont.truetype(TITLE_FONT, 40)
        name_font = ImageFont.truetype(TITLE_FONT, 70)
        stats_font = ImageFont.truetype(STATS_FONT, 30)
...
def create_nft_card(rifle, output_dir='../../public/rifles'):
    ...
    # Draw rifle stats
    stats_y = 560
    draw.text((50, stats_y), f"Scope Multiplier: {rifle['scope_multiplier']}x", font=fonts['stats'], fill="black")
    draw.text((50, stats_y + 30), f"Magazine Capacity: {rifle['mag_capacity']}", font=fonts['stats'], fill="black")
    draw.text((50, stats_y + 60), f"Reload Time: {rifle['reload_time']}s", font=fonts['stats'], fill="black")
